...
type QuorumSpec interface {
	...
	ReadQF(in *emptypb.Empty, replies map[uint32]*ReadResponse) (*ReadResponse, bool)
}

// Read is a quorum call invoked on all nodes in configuration c,
// with the same argument in, and returns a combined result.
func (c *Configuration) Read(ctx context.Context, in *emptypb.Empty) (resp *ReadResponse, err error) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "proto.StorageService.Read",
	}
	cd.QuorumFunction = func(req protoreflect.ProtoMessage, replies map[uint32]protoreflect.ProtoMessage) (protoreflect.ProtoMessage, bool) {
		...
		return c.qspec.ReadQF(req.(*emptypb.Empty), r)
	}
...
func (n *Node) Write(ctx context.Context, in *WriteRequest) (resp *emptypb.Empty, err error) {
	cd := gorums.CallData{
		Message: in,
		Method:  "proto.StorageService.Write",
	}
...
type StorageService interface {
	...
	Read(ctx gorums.ServerCtx, request *emptypb.Empty) (response *ReadResponse, err error)
}

func RegisterStorageServiceServer(srv *gorums.Server, impl StorageService) {
	srv.RegisterHandler("proto.StorageService.Write", func(ctx gorums.ServerCtx, in *gorums.Message, finished chan<- *gorums.Message) {
		req := in.Message.(*WriteRequest)
		defer ctx.Release()
		resp, err := impl.Write(ctx, req)
		gorums.SendMessage(ctx, finished, gorums.WrapMessage(in.Metadata, resp, err))
	})
	srv.RegisterHandler("proto.StorageService.Read", func(ctx gorums.ServerCtx, in *gorums.Message, finished chan<- *gorums.Message) {
